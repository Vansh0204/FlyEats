...
export default function OutletsPage() {
  ...
  useEffect(() => {
    const url = `/api/outlets?airportId=${params.id}${gateNumber ? `&gateNumber=${gateNumber}` : ''}${category ? `&category=${category}` : ''}`
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white shadow-sm">
        <div className="container mx-auto px-4 py-4 flex items-center gap-4">
          <Link href={`/airports/${params.id}`} className="text-gray-600 hover:text-orange-600">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <Link href={`/airports/${params.id}`} className="text-orange-600 hover:underline">
              Go back
            </Link>
          </div>
        ) : (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredOutlets.map((outlet) => (
              <Link
                key={outlet.id}
                href={`/outlets/${outlet.id}?airportId=${params.id}${gateNumber ? `&gate=${gateNumber}` : ''}`}
